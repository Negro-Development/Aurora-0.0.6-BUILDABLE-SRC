...
package me.halqq.aurora.client.api.gui.render.module.setting.settings
...
import me.halqq.aurora.client.api.gui.render.module.setting.SettingFrame
...
class DoubleFrame(module: ModuleFrame, x: Int, y: Int, private val setting: SettingDouble) :
    SettingFrame(module, x, y, module.width - 2, module.height) {
    private var dragging = false
    private var sliderWidth = 0
    private fun updateSlider(mouseX: Int) {
        val diff = Math.min(width, Math.max(0, mouseX - x)).toFloat()
        val min = setting.minValue
        val max = setting.maxValue
        ...
        if (dragging) {
            if (diff == 0f) {
                ...
                setting.value = newValue.toDouble()
            }
        }
    }
