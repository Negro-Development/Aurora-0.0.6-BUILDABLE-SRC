...
package me.halqq.aurora.client.api.gui.render
...
class CategoryFrame(category: Category, x: Int, y: Int) :
    ...
    init {
        ...
        if (!category.getName().equals("HudEditor")) {
            for (m in ModuleManager.INSTANCE.getModulesInCategory(category)) {
    ...
    override fun drawScreen(mouseX: Int, mouseY: Int) {
       ...
       if(!category.namee.equals("HudEditor")) TextManager.INSTANCE.drawStringWithShadow(
            category.name, (x + 1).toFloat(), (y + 2).toFloat(), -1, ModuleManager.INSTANCE.getModule(
    ...
    fun updatePosition() {
        ...
        for (cF in ClientGuiScreen.categories) {
            if (scrollWheel < 0) {
